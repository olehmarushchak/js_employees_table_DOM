...
topHeaders.forEach((header)=>{
    header.addEventListener("click", (e)=>{
        switch(e.target.textContent){
            case NAME:
                const acsSortedName = staff.sort((a, b)=>a.name.localeCompare(b.name));
                const sortedByName = isClickedName ? acsSortedName.reverse() : acsSortedName;
                table.forEach((row, index)=>{
                    ...
                    row.querySelectorAll("td")[2].textContent = sortedByName[index].office;
                    row.querySelectorAll("td")[3].textContent = sortedByName[index].age;
                    row.querySelectorAll("td")[4].textContent = sortedByName[index].salary;
                });
                isClickedName = !isClickedName;
                break;
            case POSITION:
                const acsSortedPosition = staff.sort((a, b)=>a.position.localeCompare(b.position));
                const sortedByPosition = isClickedPosition ? acsSortedPosition.reverse() : acsSortedPosition;
                table.forEach((row, index)=>{
                    ...
                    row.querySelectorAll("td")[2].textContent = sortedByPosition[index].office;
                    row.querySelectorAll("td")[3].textContent = sortedByPosition[index].age;
                    row.querySelectorAll("td")[4].textContent = sortedByPosition[index].salary;
                });
                isClickedPosition = !isClickedPosition;
                break;
            case OFFICE:
                const acsSortedOffice = staff.sort((a, b)=>a.office.localeCompare(b.office));
                const sortedByOffice = isClickedOffice ? acsSortedOffice.reverse() : acsSortedOffice;
                table.forEach((row, index)=>{
                    row.querySelectorAll("td")[0].textContent = sortedByOffice[index].name;
                    row.querySelectorAll("td")[1].textContent = sortedByOffice[index].position;
                    row.querySelectorAll("td")[2].textContent = sortedByOffice[index].office;
                    row.querySelectorAll("td")[3].textContent = sortedByOffice[index].age;
                    row.querySelectorAll("td")[4].textContent = sortedByOffice[index].salary;
                });
                isClickedOffice = !isClickedOffice;
                break;
            case AGE:
                const acsSortedAge = staff.sort((a, b)=>a.age - b.age);
                const sortedByAge = isClickedAge ? acsSortedAge.reverse() : acsSortedAge;
                table.forEach((row, index)=>{
                ...
                break;
            case SALARY:
                const acsSortedSalary = staff.sort((a, b)=>a.salary.slice(1).replace(",", "") - b.salary.slice(1).replace(",", ""));
                const sortedBySalary = isClickedSalary ? acsSortedSalary.reverse() : acsSortedSalary;
                table.forEach((row, index)=>{
                    ...
                    row.querySelectorAll("td")[2].textContent = sortedBySalary[index].office;
                    row.querySelectorAll("td")[3].textContent = sortedBySalary[index].age;
                    row.querySelectorAll("td")[4].textContent = sortedBySalary[index].salary;
                });
...
const addNotification = (statusProp)=>{
    ...
    switch(statusProp){
        ...
        case NOT_DONE:
            ...
            mainBox.textContent = "Age biggest 18 and lower 90, Name length must have 4. Maybe empty name or position";
            break;
    }
...
addedButton.addEventListener("click", (e)=>{
    ...
    for(let i = 0; i < inputs.length; i++){
        ...
        if (element.value.trim() === "") return addNotification(NOT_DONE);
    }
    const newMember = {};
    for(let i = 0; i < inputs.length; i++){
        const element = inputs[i];
